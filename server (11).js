...
const GUN_POOL = ['lmg','dmr','crossbow','blockrifle','scattergun','longshot','whisper','sidearm','mpistol','stinger','deagle','handcannon','boomtube','carbine','sawnoff','thresher','flare','popper','voltline','junker','gauss','harpoon','leech','lance','bounder'];
const MELEE_POOL = ['crowbar','hatchet','knife','sledge','katana','machete','battleaxe','chainsaw','spear'];
...
function rotateQuick(r, reason) {
  ...
  r.map = best; r.rotAt = Date.now() + QUICK_ROTATE_MS; r.p10 = false; r.p5 = false; if (r.mode === 'gun') r.ladder = makeLadder();
...
io.on('connection', (ws) => {
  ...
  function handle(m) {
    ...
    if (m.t === 'cfg' && id === r.hostId) { if (!r.quick && m.opts) { r.opts = cleanOpts(Object.assign({}, r.opts || {}, m.opts)); pushRoom(r); } if (m.gm === undefined) return; if (r.quick) { m.gm = r.mode; m.map = r.map; } else if (r.mode !== m.gm || r.map !== m.map) { if (m.gm === 'gun') r.ladder = makeLadder(); r.mode = m.gm; r.map = m.map; pushRoom(r); sendLobbies(); } toRoom(r, m, id); return; }
