...
1. Push this folder's contents to a **new GitHub repo** (root of the repo = these files directly,
   not inside a subfolder — make sure `public/index.html` genuinely ends up inside a `public` folder).

Networking runs on **Socket.IO**, not raw WebSockets — if a network blocks a real WebSocket upgrade
(some ISPs, school/office firewalls, and antivirus HTTPS-inspection do this), it automatically falls
back to disguising traffic as normal HTTPS requests instead of failing outright.
