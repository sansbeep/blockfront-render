...
const QUICK_MODES = [['waves', 'Waves', 'forest'], ['demo', 'Demolition', 'refinery'], ['ctf', 'Capture the Flag', 'canyon'], ['gun', 'Gun Game', 'compound'], ['ffa', 'Free For All', 'city']];
const GUN_LEVELS = 16;
...
function leaveRoom(id) {
  ...
  if (r.mode === 'ctf' && r.teams) { const dropped = ctfDrop(r, id, c.lastPos); r.teams.delete(id); if (r.players.size) { ctfBalance(r); ctfPush(r, dropped ? { type: 'drop', by: id } : null); } }
...
function joinRoom(id, r) {
  ...
  leaveRoom(id); c.room = r.id; c.ready = false; r.players.add(id); if (r.hostId === null && !r.hub) r.hostId = id; if (r.mode === 'ctf') ctfAssign(r, id);
  send(c.ws, { t: 'welcome', id, host: id === r.hostId });   // (re)announce host status in this room
  pushRoom(r); if (r.mode === 'ctf') ctfPush(r); sendLobbies(); log(`${c.name} joined "${r.name}" (${r.players.size})`);
}
...
const CTF_TARGET = 3, CTF_RETURN_MS = 25000, CTF_TIME_MS = 10 * 60000, CTF_SWAP_MS = 20000;
...
const ctfState = (r, ev) => ({ t: 'ctf', teams: Object.fromEntries([...r.teams].filter(([id]) => r.players.has(id))), flags: r.flags.map(f => ({ s: f.s, by: f.by, pos: f.pos })), caps: r.caps, target: CTF_TARGET, ends: r.ctfStart + CTF_TIME_MS, ev: ev || null });
...
setInterval(() => {   // dropped flags go home on their own; swaps that waited too long happen anyway; the clock runs out
  for (const r of rooms.values()) { if (r.mode !== 'ctf' || !r.flags) continue; let ch = false;
...
function rotateQuick(r, reason) {
  ...
  if (r.mode === 'ctf') ctfReset(r, false);
  for (const id of r.players) { const c = clients.get(id); if (c) { c.k = 0; c.d = 0; c.s = 0; c.vote = null; } }
  toRoom(r, { t: 'rotate', mode: r.mode, map: r.map, reason }); pushRoom(r); if (r.mode === 'ctf') ctfPush(r, { type: 'new' }); sendLobbies(); log(`${r.name} → ${r.map} (${reason}${bestN ? ', ' + bestN + ' votes' : ''})`);
}
...
io.on('connection', (ws) => {
  ...
  function handle(m) {
    ...
    if (r.mode === 'ctf' && r.flags) {
      ...
      if (m.t === 'fg') { const fi = m.f | 0, f = r.flags[fi]; if (!f || my === undefined || fi === my || f.s === 'carried' || r.flags.some(x => x.by === id)) return;
        f.s = 'carried'; f.by = id; f.pos = null; return ctfPush(r, { type: 'grab', by: id, f: fi }); }
      if (m.t === 'fr') { const fi = m.f | 0, f = r.flags[fi]; if (!f || my !== fi || f.s !== 'dropped') return; f.s = 'home'; f.pos = null; credit(c, 5, 0, 'FLAG RETURNED'); return ctfPush(r, { type: 'return', by: id, f: fi }); }
      if (m.t === 'fc') { if (my === undefined) return; const ef = r.flags[1 - my]; if (ef.s !== 'carried' || ef.by !== id || r.flags[my].s !== 'home') return;
        ...
        ctfPush(r, { type: 'cap', by: id, team: my }); if (r.caps[my] >= CTF_TARGET) ctfEnd(r, my); return; }
      if (m.t === 'fd') { if (ctfDrop(r, id, m.pos)) ctfPush(r, { type: 'drop', by: id }); return; }
      if (m.t === 'dmg' && typeof m.to === 'number' && my !== undefined && r.teams.get(m.to) === my) return;   // no friendly fire, enforced here too
    }
    if (m.t === 'died') {
      if (r.mode === 'ctf' && r.flags) { if (ctfDrop(r, id, c.lastPos)) ctfPush(r, { type: 'drop', by: id }); if (c.ctfMove) { ctfSwap(r, id, c); ctfPush(r); } }
