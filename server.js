...
const QUICK_MODES = [['waves', 'Waves', 'forest'], ['demo', 'Demolition', 'refinery'], ['koth', 'King of the Hill', 'canyon'], ['gun', 'Gun Game', 'compound'], ['ffa', 'Free For All', 'city']];
const GUN_LEVELS = 16;
...
const GUN_POOL = ['lmg','dmr','crossbow','blockrifle','scattergun','longshot','whisper','sidearm','mpistol','stinger','deagle','handcannon','boomtube','carbine','sawnoff','thresher','flare','popper','voltline','junker','gauss','harpoon','leech','lance','bounder','kestrel','drifter'];
const MELEE_POOL = ['crowbar','hatchet','knife','sledge','katana','machete','battleaxe','chainsaw','spear','tomahawk'];
// Weapons newer than the V75 client. A client announces what it has with {t:'caps'}; one that never does is an older build,
// so it only ever gets ladder weapons it knows. (Older clients fall back to their own local ladder on an unknown id,
// which would leave them playing different weapons from everyone else in the room.)
const NEW_WEAPONS = new Set(['kestrel', 'drifter', 'tomahawk']);
const canUse = (c, w) => !NEW_WEAPONS.has(w) || !!(c && c.caps && c.caps.has(w));
const roomCan = (r, w) => { if (!r) return !NEW_WEAPONS.has(w); for (const id of r.players) if (!canUse(clients.get(id), w)) return false; return true; };
function makeLadder(r) { const a = GUN_POOL.filter(w => roomCan(r, w)); for (let i = a.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [a[i], a[j]] = [a[j], a[i]]; } const mp = MELEE_POOL.filter(w => roomCan(r, w)); return a.concat([mp[Math.floor(Math.random() * mp.length)]]); }
// Someone joined who is missing a weapon on the current ladder: swap just those rungs, in place, so nobody's level changes.
function fitLadder(r) { if (!r || r.mode !== 'gun' || !r.ladder) return false; let changed = false; const L = r.ladder, last = L.length - 1;
  for (let i = 0; i < L.length; i++) { if (roomCan(r, L[i])) continue; const src = (i === last ? MELEE_POOL : GUN_POOL).filter(w => roomCan(r, w));
    // prefer a weapon not already on the ladder; the full gun pool is usually all used, so fall back to a repeat (never beside itself)
    let pool = src.filter(w => !L.includes(w)); if (!pool.length) pool = src.filter(w => w !== L[i - 1] && w !== L[i + 1]);
    if (pool.length) { L[i] = pool[Math.floor(Math.random() * pool.length)]; changed = true; } }
  return changed; }
...
const io = new Server(server, { cors: { origin: '*' }, transports: ['polling', 'websocket'], pingTimeout: 60000, pingInterval: 25000, perMessageDeflate: false, httpCompression: false });
...
const toRoomFast = (r, o, except) => { for (const id of r.players) if (id !== except) { const c = clients.get(id); if (c) c.ws.volatile.emit('msg', o); } };
...
const roomState = r => ({ t: 'room', id: r.id, name: r.name, quick: !!r.quick, hub: !!r.hub, duel: !!r.duel, opts: r.opts || undefined, votes: r.quick ? voteTally(r) : undefined, ladder: r.mode === 'gun' ? (r.ladder || (r.ladder = makeLadder(r))) : undefined, banned: r.banned || [], host: r.hostId, mode: r.mode, map: r.map, playing: r.playing, nextIn: r.quick ? Math.max(0, Math.round((r.rotAt - Date.now()) / 1000)) : null,
...
function joinRoom(id, r) {
  ...
  leaveRoom(id); c.room = r.id; c.ready = false; r.players.add(id); if (r.hostId === null && !r.hub) r.hostId = id; fitLadder(r);
...
function rotateQuick(r, reason) {
  ...
  r.map = best; r.rotAt = Date.now() + QUICK_ROTATE_MS; r.p10 = false; r.p5 = false; r.startedAt = Date.now(); r.paidWave = 0; r.paidRound = 0; r.paidBoss = false; if (r.mode === 'gun') r.ladder = makeLadder(r);
...
io.on('connection', (ws) => {
  ...
  function handle(m) {
    ...
    if (m.t === 'caps') { if (Array.isArray(m.w)) { c.caps = new Set(m.w.filter(x => typeof x === 'string').slice(0, 400)); const r0 = roomOf(id); if (r0 && fitLadder(r0)) pushRoom(r0); } return; }
    ...
    if (m.t === 'died') {
      ...
      if (killer && m.by !== id && killer.room === r.id) { killer.k++; killer.s += 100; if (killer.user && users[killer.user]) { users[killer.user].k++; saveUsers(); }
        ...
        if ((r.mode === 'ffa' || r.mode === 'gun') && killer.k >= (r.duel ? 5 : r.mode === 'gun' ? (r.ladder ? r.ladder.length : GUN_LEVELS) : (r.opts && r.opts.target) || FFA_TARGET)) { endMatch(r, m.by); if (r.mode === 'gun' && !r.quick) { r.ladder = makeLadder(r); pushRoom(r); } if (r.quick) rotateQuick(r, 'match won');
    ...
    if (m.t === 's') { toRoomFast(r, m, id); return; }   // m.id is already stamped above
    toRoom(r, m, id);
  }
  ws.on('disconnect', () => {
