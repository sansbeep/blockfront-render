...
const MAP_POOL = ['forest', 'city', 'refinery', 'wreck', 'compound'];
const QUICK_MODES = [['waves', 'Waves', 'forest'], ['demo', 'Demolition', 'refinery'], ['inf', 'Infiltration', 'compound'], ['ffa', 'Free For All', 'city']];
...
const GRACE_MS = 30000;   // a dropped player keeps their seat this long; signing back in with the same name resumes it
...
io.on('connection', (ws) => {
  ...
  function handle(m) {
    ...
    if (m.t === 'duel' && m.act === 'acc') {   // accepter (id) + challenger (m.to) get their own 1v1 arena, first to 5
      ...
      const maps = ['city', 'forest', 'compound', 'refinery', 'wreck']; const d = { id: ++rseq, name: `${o.name} vs ${c.name}`, hostId: m.to, mode: 'ffa', map: maps[Math.floor(Math.random() * maps.length)], playing: true, duel: true, players: new Set(), banned: [] };
    ...
    if (m.t === 'score' && id === r.hostId) { const w = clients.get(m.who); if (w && w.room === r.id) { w.k += m.k || 0; w.s += m.pts || 0; if (w.user && users[w.user]) { users[w.user].k += m.k || 0; saveUsers(); } credit(w, (m.pts || 0) >= 250 ? 8 : 3, 0); pushRoom(r); } return; }
    ...
    if (m.t === 'died') {
      ...
      if (killer && m.by !== id && killer.room === r.id) { killer.k++; killer.s += 100; if (killer.user && users[killer.user]) { users[killer.user].k++; saveUsers(); }
        ...
        if (r.mode === 'ffa' && killer.k >= (r.duel ? 5 : FFA_TARGET)) { endMatch(r, m.by); if (r.quick) rotateQuick(r, 'match won');
