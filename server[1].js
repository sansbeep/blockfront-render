...
const walletOf = u => { u.blocks = u.blocks ?? 300; u.prisms = u.prisms ?? 0; u.owned = u.owned || []; return u; };
...
const ROTATION = [['waves','forest'],['ffa','city'],['inf','compound'],['demo','refinery'],['ffa','wreck'],['waves','city'],['inf','forest'],['demo','forest'],['ffa','compound'],['waves','wreck']];
...
io.on('connection', (ws) => {
  const id = ++seq; const c = { ws, name: 'Player ' + id, user: null, k: 0, d: 0, s: 0, room: null, ready: false }; clients.set(id, c);
  ...
  function handle(m) {
    ...
    if (m.t === 'wv' && id === r.hostId) { creditRoom(r, 15, (m.w === 10 && !r.p10) ? 10 : 0, 'WAVE ' + m.w + ' CLEARED'); if (m.w === 10) r.p10 = true; toRoom(r, m, id); return; }
    if (m.t === 'rnd' && id === r.hostId) { creditRoom(r, 25, (m.r === 6 && !r.p5) ? 10 : 0, 'ROUND ' + (m.r - 1) + ' CLEARED'); if (m.r === 6) r.p5 = true; toRoom(r, m, id); return; }
    ...
    if (m.t === 'score' && id === r.hostId) { const w = clients.get(m.who); if (w && w.room === r.id) { w.k += m.k || 0; w.s += m.pts || 0; if (w.user && users[w.user]) { users[w.user].k += m.k || 0; saveUsers(); } credit(w, (m.pts || 0) >= 250 ? 6 : 2, 0); pushRoom(r); } return; }
    if (m.t === 'buy') { const u = c.user && users[c.user]; const pr = PRICES[m.id]; if (!u || !pr) return; walletOf(u); if (u.owned.includes(m.id)) return sendWallet(c); if ((pr.b && u.blocks < pr.b) || (pr.p && u.prisms < pr.p)) return sendWallet(c, 'NOT ENOUGH ' + (pr.p ? 'PRISMS' : 'BLOCKS')); u.blocks -= pr.b || 0; u.prisms -= pr.p || 0; u.owned.push(m.id); saveUsers(); sendWallet(c, 'UNLOCKED'); return; }
    if (m.t === 'died') {
  ...
  ws.on('disconnect', () => { leaveRoom(id); clients.delete(id); sendLobbies(); sendFriends(); log(`${c.name} disconnected (${clients.size} online)`); });
});
